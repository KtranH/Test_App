<template>
  <header class="sticky top-0 z-40 h-14 border-b border-black/10 bg-white/60 backdrop-blur supports-[backdrop-filter]:bg-white/50">
    <div class="h-full flex items-center justify-between px-4">
      <div class="flex items-center gap-2">
        <component :is="isSidebarOpen ? X : Menu" class="h-5 w-5 cursor-pointer" @click="toggleSidebar" />
        <span class="font-medium tracking-tight">Quản trị</span>
      </div>
      <div class="flex items-center gap-3">
        ...
        <button
          type="button"
          class="h-9 px-3 rounded-lg bg-black/5 text-black/70 hover:bg-black/10 focus-visible:ring-2 focus-visible:ring-black/20 inline-flex items-center gap-2"
          @click="resetDemo"
        >
