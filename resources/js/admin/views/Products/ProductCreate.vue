    ...
    <form class="grid grid-cols-1 lg:grid-cols-3 gap-6" @submit.prevent="save">
      <div class="lg:col-span-2 space-y-4">
        <div class="border border-black/10 rounded-lg p-4 space-y-3">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
              ...
              <input v-model="form.name" class="w-full px-3 py-2 border rounded-lg" placeholder="Tên sản phẩm" />
            </div>
            <div>
              <label class="text-xs text-black/60">Slug</label>
              <input v-model="form.slug" class="w-full px-3 py-2 border rounded-lg" placeholder="slug" />
            </div>
          </div>
          <div>
        ...
        <div class="border border-black/10 rounded-lg p-4">
          <VariantGenerator :product-id="productId" @generate="onGenerate" />
        </div>

        <VariantTable :product-id="productId" />
      </div>
...
const save = () => {
  productStore.createProduct({ id: productId.value, ...form.value })
  router.push({ name: 'admin.products' })
}

const cancel = () => router.push({ name: 'admin.products' })

const onGenerate = (generated) => {
  generated.forEach(g => productStore.upsertVariant({ ...g, productId: productId.value }))
}
