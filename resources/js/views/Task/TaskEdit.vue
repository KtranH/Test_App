        ...
        <div class="px-4 sm:px-0">
          <div class="card max-w-full">
            <form class="space-y-6" @submit.prevent="onSubmit">
              <h3 class="text-sm font-semibold text-gray-500 tracking-wide">
                Thông tin chung
              </h3>
              <div>
              ...
              <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div class="relative col-span-2" ref="userBox">
                  ...
                  <p v-if="selectedUser" class="mt-1 text-xs text-gray-500">
                    Đã chọn: {{ selectedUser.name }} (ID: {{ selectedUser.id }})
                  </p>
                  <p v-else-if="invalidUser" class="mt-1 text-xs text-red-600">
                    Vui lòng chọn người dùng từ gợi ý
                  </p>
                </div>
              </div>
              <h3 class="text-sm font-semibold text-gray-500 tracking-wide">
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  />
                  <p v-if="isDateInvalid" class="mt-1 text-xs text-red-600">
                    Ngày kết thúc không được trước ngày bắt đầu
                  </p>
                  <p v-if="isStartTooEarly" class="mt-1 text-xs text-red-600">
                    Ngày bắt đầu không được nhỏ hơn hiện tại
                  </p>
                </div>
              </div>
              <div
                class="flex items-center justify-end gap-3 pt-2 border-t border-gray-100 mt-2 pt-4"
              >
                <router-link to="/tasks" class="btn-secondary">Hủy</router-link>
                <button
                  :disabled="taskStore.isSubmitting || !isFormValid"
                  type="submit"
                  class="btn-primary"
                >
...
onMounted(async () => {
  ...
  form.start_date = toDateOnly(task.start_date);
  form.end_date = toDateOnly(task.end_date);
  // hiển thị tên user hiện tại nếu có
  if (task.user && task.user.name) {
    userSearch.value = `${task.user.name} (${task.user.email || "no-email"})`;
  }
  loaded.value = true;
});
...
const userResults = computed(() =>
  Array.isArray(userStore.userQuery) ? userStore.userQuery : []
);
...
const invalidUser = computed(() => {
  return !(
    form.user_id &&
    Number.isInteger(Number(form.user_id)) &&
...
const minStart = new Date().toISOString().slice(0, 10);
